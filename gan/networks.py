...
class UpSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        ##################################################################
        # Step 1 -> ref: https://pytorch.org/docs/stable/generated/torch.repeat_interleave.html 
        x_upsampled = x.repeat_interleave(int(self.upscale_factor**2),dim=1)
        
        # Step 2 (worked better than nn.PixelShuffle) 
        x_shuffled = F.pixel_shuffle(x_upsampled, self.upscale_factor)
...
class DownSampleConv2D(torch.jit.ScriptModule):
    def __init__(
        ...
        self.conv = nn.Conv2d(
            input_channels, n_filters, kernel_size, padding=padding
        ) # check in_channels=input_channels? (replace it?)
    ...
    def forward(self, x):
        ...
        a,b,c,d = torch.chunk(x_unshuffled, 4, dim = 1)
        output = (a+b+c+d)/4
...
class ResBlockUp(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        res = x
        x = self.layers(x)
        return self.upsample_residual(res) + x
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class ResBlockDown(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        res = x
        x = self.layers(x)
        return self.downsample_residual(res) + x
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Discriminator(torch.jit.ScriptModule):
    ...
    def __init__(self):
        ...
        self.layers = nn.Sequential(
            ResBlockDown(input_channels=3, n_filters=128),    # in_channels = 3 (for RGB images)
    ...
    def forward(self, x):
        ...
        ##################################################################
        x = self.layers(x)
        x = x.sum(dim=(2,3)).view(-1,128) # Summation over the image's dims and reshape
        return self.dense(x)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
